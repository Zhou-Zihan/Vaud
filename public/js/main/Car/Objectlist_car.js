...
function showobjectlist_car(obj){
    ...
    var thiscardiv =d3.select("#map_objectlistdiv").append("div")
        ...
        .on("mousemove",function(){
            if(obj.ismove==0){
                ...
                d3.selectAll("#car_traj"+obj.data.ID).transition().ease("linear")
                        .duration(300).delay(0).style("opacity","1")
            }
        })
    ...
    var speed=0,passageintime=0,runningtime=1;
    ...
    var attrdiv=thiscardiv.append("div").attr("class","attr")
        .style("pointer-events","none")
    attrdiv.append("div")
        .style("pointer-events","none")
    ...
    attrdiv.append("div")
        .style("pointer-events","none")
        .text("Riding duration  :   "+parseInt(runningtime/120)+"h"+parseInt((runningtime/2)%60)+"min")
    attrdiv.append("div")
        .style("pointer-events","none")
        .text("Duration with passengers:   "+parseInt(passageintime/120)+"h"+parseInt((passageintime/2)%60)+"min")
}
...
function showobjectlist_carlist(obj){
    ...
    var thiscarlistdiv=d3.select("#map_objectlistdiv").append("div")
        ...
        .on("mousemove",function(){
            if(obj.ismove==0){
                ...
                d3.selectAll("#carlist_traj"+obj.id).transition().ease("linear")
                        .duration(300).delay(100)
                        .style("opacity",d3.select("#carlist_traj"+obj.id).attr("opacity"))
            }
        })
        .on("mouseout",function(){
            obj.ismove=0;
    ...
    thiscarlistdiv.append("img")
        ...
        .on("mouseup",function(){
            ...
            d3.select("#objlistcarlistdiv"+d3.select(this).attr("thisid")).remove();
        })
    //?????????
    var attrdiv=thiscarlistdiv.append("div")
        .style("pointer-events","none").attr("class","attr").style("position","absolute").style("top","17px")
    attrdiv.append("div")
        .style("pointer-events","none").text("number : "+obj.data.length)
