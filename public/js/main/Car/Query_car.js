...
function querycar_old(count) {
		...
		for (var i = 0; i < condition.length; i++) {
			...
			if (thiscondition.type == 'what') {
				if (thiscondition.data[0] == 'speed') {
					sqlstring +=
						"(speed between '" +
						thiscondition.data[1].split('~')[0] +
						"'' and '" +
						thiscondition.data[1].split('~')[1] +
						"'')"
				}
			}
			if (thiscondition.type == 'where') {
				...
				thissqlstring =
					"where (date='20140114') and ( ((minlo between '" +
					thiscondition.data[1] +
					"' and '" +
					thiscondition.data[3] +
					"')and (minla between '" +
					thiscondition.data[0] +
					"' and '" +
					thiscondition.data[2] +
					"'))" +
					'or' +
					"((minlo between '" +
					thiscondition.data[1] +
				...
				QueryDb.getcarbygeoindex(thissqlstring, null, function (data) {
					console.log(data)
					...
					log(
						'Search taxis from ' +
							d3.select('#nodenamenode' + count)[0][0].outerText
					)
			...
			if (thiscondition.type == '+') {
				if (thiscondition.data[0].type == '+') {
					sqlstring =
						"where (Taxi_name='T0230' or 'T1168') and (time between '2014-01-14 00:00' and '2014-01-14 24:00') "
				}
				...
				for (var i = 0; i < thiscondition.data.length; i++) {
					...
					if (thiscondition.data[i].type == 'where') {
						...
						thissqlstring =
							"where  (date='20140114') and ( ((minlo between '" +
							thiscondition.data[i].data[1] +
							"' and '" +
							thiscondition.data[i].data[3] +
							"')and (minla between '" +
							thiscondition.data[i].data[0] +
							"' and '" +
							thiscondition.data[i].data[2] +
							"'))" +
							'or' +
							"((minlo between '" +
							thiscondition.data[i].data[1] +
						...
						QueryDb.getcarbygeoindex(
							...
							function (data) {
								...
								show_data_nodetip(lastnode, 'record')
								nodelist.getlistiditem(
									'node' + count
								).showdetail = false
								hide_condition_nodedetail(
									d3.select('#node' + count)
								)
								...
								log(
									'Search taxis from ' +
										d3.select('#nodenamenode' + count)[0][0]
											.outerText
								)
							}
						)
					...
					if (thiscondition.data[i].type == 'speed') {
						sqlstring +=
							"(speed between '" +
							thiscondition.data[i].data[1].split('~')[0] +
							"'' and '" +
							thiscondition.data[i].data[1].split('~')[1] +
							"'')"
					}

					if (i == thiscondition.data.length - 1) sqlstring += ')'
				}
			}
		}
	} else {
	...
	if (!flagwhere) querycarnormal(sqlstring, count)
}
...
function car_handleresult(data, count) {
	...
	tempnode.setdatalist(tempcar)
	// if (thiscase == 1 && casestep == 3) {
	// 	debugger
	// 	tempnode.timearea = [
	// 		'2014-1-01 ' + sqlobject.time[0].substr(0, 5),
	// 		'2014-1-01 ' + sqlobject.time[1].substr(0, 5),
	// 	]
	// }
