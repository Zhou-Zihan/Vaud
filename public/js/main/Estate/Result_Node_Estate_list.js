...
function  show_estatelist(node) {
    ...
    d3.select("#sourcediv" + node.attr("id")).append("div")
        ...
        .on("mouseup", function () {
            ...
            add_mapobject("estatelist",data,node.attr("count"))
        })
        .on("mousemove", function () {
            if ((d3.event.x - mousex > 5 || d3.event.y - mousey > 5)
                &&isabledrag_new_condition) {
                isabledrag_new_condition=false;
                new_conditiondiv("estatelist",data,d3.event.x,d3.event.y,activenode,1);
            }
        }).text("Select All")
    ...
    if (data != null) {
        ...
        for (var i = 0; i < data.length&&i<50; i++) {
            var thiselementdiv = sourcedivlist.append("div")
                ...
                .attr("estatename", data[i].detailname)
                .attr("intid",i)
                .text(data[i].detailname)
            //triangle
            ...
            thiselementdiv
                .on("mousedown", function () {
                    map.setView([data[d3.select(this).attr("intid")].latitude, 
                        data[d3.select(this).attr("intid")].longitude], 13);

                    mousex = d3.event.x;
                    mousey = d3.event.y;
                    isabledrag_new_condition=true;
                    add_mapobject("estate",data[d3.select(this).attr("intid")],node.attr("count"))
                })
                .on("mouseup", function () {
                    isabledrag_new_condition=false;
                })
                .on("mousemove", function () {
                    if ((d3.event.x - mousex > 5 || d3.event.y - mousey > 5)
                        &&isabledrag_new_condition) {
                        isabledrag_new_condition=false;
                        new_conditiondiv("estate",data[d3.select(this).attr("intid")],d3.event.x,d3.event.y,activenode,1);
                    }
                });
            ;
        }
