...
function paint_peopletraj(people) {
//460022584127733
    /******************************************
     *       load data
     *******************************************/
...
function highlightpaint_peopletraj(people){
    ...
    for (var i = 0; i < data.length; i++) {
        if(data[i].time<timearea[1]&&
            data[i].time>timearea[0]){
            var p=map.latLngToContainerPoint(
    ...
    if(people.highlight!=0) {
         var highlightpeople = d3.select("#sceneviewfold")
            .append("div")
            ...
            .on("mousedown",function(){
                d3.select("body").append("svg")
                    ...
                    .on("mousemove",function(){
                        highlightpeople.style("left",(d3.event.x-150)+"px")
        ...
        highlightpeople.append("div")
            ...
            .on("mousedown", function () {
                ...
                for (var i = 0; i < people.data.length; i++) {
                    if(people.data[i].time<timearea[1]&&people.data[i].time>timearea[0]){
                        positionselection.push({type:people,nodecount:people.node,data:people.data[i]})
                    }
                }
...
function paint_peoplelisttraj(peoplelist){
    ...
    for(var num=0;num<data.length;num++) {
        ...
        for (var i = 0; i < data[num].pInfo.length; i++) {
            if(data[num].pInfo[i].latitude>0&&data[num].pInfo[i].longitude>0){
                     if(data[num].pInfo[i].time<=timearea[1]&&
                        data[num].pInfo[i].time>=timearea[0]){
                        isintime=true;
                        var p = map.latLngToContainerPoint(
                            L.latLng(data[num].pInfo[i].latitude, data[num].pInfo[i].longitude)
                        )
                        latlngs = latlngs + p.x + "," + p.y + " ";
                    }
             }
        ...
        if(isintime){
            ...
            var polyline = d3.select("#mapobjectsvg")
                ...
                .on("mouseup",function(){
                    ...
                    add_mapobject("people",d3.select(this).attr("peopleid"),peoplelist.node)
                });
            if (peoplelist.ismove != 0) {
                var polylineupper = d3.select("#mapobjectsvg")
                    .append("polyline")
                    .attr("points", polylinelist.path);
                polylineupper.attr("class", "peopletraj peopletrajismove peoplelisttrajpolyline");
                polyline.attr("class", "peopletraj peopletrajismovebg peoplelisttrajpolyline");
            }
        ...
        var alpha=1;
         ...
         if(thislistnum>500)
                alpha=0.05;
        d3.select("#mapobjectsvg").selectAll(".peoplelisttrajpolyline").style("stroke-opacity",alpha)
        ...
        highlightpeople.append("div")
            ...
            .on("mousedown",function(){
                d3.select("body").append("svg")
                    ...
                    .style('height',"100%")
                    .on("mousemove",function(){
                        highlightpeople.style("left",(d3.event.x-95)+"px")
                            .style("top",(d3.event.y-15)+"px")
                    })
                    .on("mouseup",function(){
                        d3.select(this).remove();
...
function repaint_map_peoplepositionselection() {
    ...
    for (var i = 0; i < positionselection.length;i++){
        ...
        var latlon=[positionselection[i].data.lat,positionselection[i].data.lon]
        ...
        d3.select("#mapobjectsvg")
            ...
            .on("mousedown", function () {
               ...
               highg.append("text").attr("class","carpoint")
                ...
                highg.append("text")
                    .attr("class","carpoint")
                    .text("Time:   "+parseInt(positionselection[d3.select(this).attr("count")].data.time/100)
                    +" : "+parseInt((positionselection[d3.select(this).attr("count")].data.time)%100))
                    .attr("x",highg.attr("x")*1+10)
                    ...
                    .on("mousedown",function(){
                        var data=positionselection[d3.select(this).attr("count")];
                        data.data.time=parseInt(positionselection[d3.select(this).attr("count")].data.time/100)*60
                            +parseInt((positionselection[d3.select(this).attr("count")].data.time)%100)
                        console.log(data.data)
                        new_conditiondiv("timepoint",data.data,d3.event.x,d3.event.y,data.nodecount,1);
                    })
