...
function querypeople_old(count) {
	...
	if (where.length == 0) {
		where = [[27.0, 119.649, 29.01343, 121.0]]
	}
	...
	if (condition.length > 0) {
		if (condition[0].type == '+') {
			...
			if (thiscase == 1 && thiscasecount == 0) {
				...
				qm.getMultiWhoTraj(['460022584127733'], 1000, function (data) {
					console.log(data)
					data_node_newnode('people', [
						d3
				...
				qm.getWhichByMulti(where, when, 1, 'union', count, function (
					data
				) {
					...
					if (which.length != 0) {
						qm.getMultiWhoTraj(which, 1000, function (data) {
							data_node_newnode('people', [
								d3
									.select('#nodediv' + count)
									.style('left')
...
function people_handleresult(data, count) {
	...
	data.forEach((o) => {
		if (!temppeople.has(o.id)) temppeople.set(o.id, { ID: o.id, pInfo: [] })
