function condition_reconode_newnode(reconode){
   ...
   /**************************
    ...
    newnode.setid("reconode"+reconode.id)
    ...
    var reconodediv = d3.select("#reconodelist")
        ...
        .on("mouseover", function(){
            linepaint_for_reco(reconode.id);
            show_result_heatmap(reconode.id);
        })
    ...
    reconodediv.append("div")
        ...
        .style("width","15%")
        .style("height","30px")
        .style("background","url(image/map_objlist_"+reconode.type+"_normal.svg) no-repeat")
    ...
    reconodediv.append("div")
        ...
        .on("mousedown",function(){
            new_reco_drag(reconode.id,reconode.resultnum,reconode.type,d3.event.x,d3.event.y);            
        });
    
    show_reco_conditionlist(reconode.id);
}
...
function new_reco_drag(count,num,type,mousex,mousey){
        ...
        .on("click",function(){
            ...
            for(var i= 0;i<pointlist.length;i++){
                if(pointlist[i].son==count){
                    for(key in pointlist[i].father){
                        nodelist.pushfather_and_son({
                            father: pointlist[i].father[key],
    ...
    thisnewdiv.append("img")
        ...
        .attr("height","30px")
        .attr("width","30px")
        .attr("src","image/map_objlist_"+type+"_normal.svg")
