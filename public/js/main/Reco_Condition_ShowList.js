function show_reco_conditionlist(id) {
	...
	var conditionlist = d3
		.select('#reconodediv' + id)
		.append('div')
...
function addrecoconditionlist_inter(conditionlist, conditions, i, id) {
	var thisitem = conditionlist
		.append('div')
		.attr('id', 'recoconditionlist' + id + '_item' + i)
		.attr('count', i)
	...
	thisitem
		...
		.on('mousedown', function () {
			...
			if (d3.select(this).attr('show') == 'false') {
				...
				var thisconditiondiv = d3.select(
					'#recoconditionlist' +
						id +
						'_item' +
						d3.select(this).attr('count')
				)

				for (var l = 0; l < conditions.data.length; l++) {
					if (conditions.data[l].type == 'time') {
						addrecoconditionlist_time(
							thisconditiondiv,
							conditions.data[l],
							d3.select(this).attr('count') + l,
							id
						)
					}

					if (conditions.data[l].type == 'where') {
						addrecoconditionlist_where(
							thisconditiondiv,
							conditions.data[l],
							d3.select(this).attr('count') + l,
							id
...
function addrecoconditionlist_where(conditionlist, condition, idx, id) {
	var thisitem = conditionlist
		...
		.attr('class', 'recocondition_conditionitem')
		.text(function () {
			if (condition.data[4] != null) return 'Region '
	...
	thisitem
		...
		.on('mousedown', function () {
			...
			if (
				d3.select('#reco' + id + 'thisitem' + idx).style('height') ==
				'14px'
			) {
				d3.select('#reco' + id + 'thisitem' + idx).style(
					'height',
					'114px'
				)
				...
				var thismap_div = d3
					...
					.style('height', '100px')
					.style('width', '200px')
				var map1 = L.map('reco' + id + 'map_condition' + idx).setView(
					[whereitem[2], whereitem[1]],
					12
				)
				L.tileLayer(
...
function addrecoconditionlist_what(conditionlist, condition, i, id) {
	...
	thisitem
		...
		.text(function () {
			...
			else return condition.data[0] + ' = ' + condition.data[1]
		})
		.style('display', 'inline-block')
		.style('position', 'absolute')
		.style('width', '180px')
