...
d3.select('body')
	.on('mousemove', function () {
		if (nodedrag.boolean) {
			d3.select('#nodediv' + nownode).style(
				'left',
				d3.event.x - nodedrag.mousex + 'px'
			)
			d3.select('#nodediv' + nownode).style(
				'top',
				d3.event.y - nodedrag.mousey + 'px'
			)
			if (!nodelist.getlistiditem('node' + nownode).showdetail) {
				d3.select('#node_cricle' + nownode).style(
					'left',
					d3.event.x - nodedrag.mousex + 'px'
				)
				d3.select('#node_cricle' + nownode).style(
		...
		if (nodesvgmove.boolean) {
			for (var i = 0; i < nodelist.getlist().length; i++) {
				...
				if (!nodelist.getlistiditem('node' + count).showdetail) {
					d3.select('#node_cricle' + count).style(
						'left',
	...
	.on('mouseup', function () {
		if (nodedrag.boolean) {
			...
			d3.select('#nodediv' + nownode).attr(
				'x',
				d3.event.x - nodedrag.mousex
			)
			d3.select('#nodediv' + nownode).attr(
				'y',
				d3.event.y - nodedrag.mousey
			)
			if (!nodelist.getlistiditem('node' + nownode).showdetail) {
				d3.select('#node_cricle' + nownode).attr(
					'x',
					d3.event.x - nodedrag.mousex
				)
				d3.select('#node_cricle' + nownode).attr(
		...
		if (nodesvgmove.boolean) {
			for (var i = 0; i < nodelist.getlist().length; i++) {
				...
				if (!nodelist.getlistiditem('node' + count).showdetail) {
					d3.select('#node_cricle' + count).attr(
						'x',
						d3.select('#node_cricle' + count).attr('x') -
							nodesvgmove.mousex +
							d3.event.x
...
function linepaint() {
	...
	for (var i = 0; i < pointlist.length; i++) {
		if (
			nodelist.getlistiditem('node' + pointlist[i].father).islive &&
			nodelist.getlistiditem('node' + pointlist[i].son).islive
		) {
			//compute x y
			if (
				nodelist.getlistiditem('node' + pointlist[i].father).showdetail
			) {
				var fatherx =
						d3
							.select('#nodediv' + pointlist[i].father)
							.style('left')
							.split('px')[0] -
						1 +
						286,
			...
			if (nodelist.getlistiditem('node' + pointlist[i].son).showdetail) {
				var sonx =
						d3
							.select('#nodediv' + pointlist[i].son)
							.style('left')
							.split('px')[0] - 10,
					sony =
						d3
			...
			if (fathersonx < 0) fathersonx = 0 - fathersonx
			if (fathersony < 0) fathersony = 0 - fathersony
			...
			//paint line
			var fatherType = nodelist
				.getlistiditem('node' + pointlist[i].father)
				.getnodetype()
			var sonType = nodelist
				.getlistiditem('node' + pointlist[i].son)
				.getnodetype()

			if (fatherType == 'condition' && sonType == 'data') {
				d3.select('#queryviewsvg')
					.append('path')
				...
				d3.select('#queryviewsvg')
					...
					.attr(
						'style',
						'stroke:#858585; fill:none; stroke-width:2; stroke-dasharray:8 8'
					)
					.attr(
						'd',
						'M' +
...
function query(count, isfrom_reco_result) {
	...
	if (typeof isfrom_reco_result !== 'undefined') {
		...
		if (thisnode.type == 'car') car_handleresult(isfrom_reco_result, count)
		...
		if (thisnode.type == 'blog') sqlobject = queryblog(count)

		if (thisnode.type == 'car') sqlobject = querycar(count)

		if (thisnode.type == 'people') sqlobject = querypeople(count)

		if (thisnode.type == 'estate') queryestate(count)

		if (thisnode.type == 'point_of_interest') sqlobject = querypoi(count)

		if (thisnode.type == 'social_network') querysocialnetwork(count)

		/*******************************
		 *   query weather
		 *******************************/
		if (thisnode.type == 'weather') {
			qm.getWeather('2014-01-01', nodelist.getlistlength() - 1, function (
				data
			) {
				data_node_newnode('estate', [
					d3
						.select('#nodediv' + count)
						.style('left')
						.split('px')[0] -
						1 +
						400,
				...
				var tempnode = nodelist.getlistindexof(
					nodelist.getlistlength() - 1
				)
				tempnode.setdatalist(data)
				nodelist.changelistiditem(
					nodelist.getlistlength() - 1,
					tempnode
				)
	...
	for (var i = 0; i < pointlist.length; i++) {
		if (pointlist[i].son == id) {
			...
			if (nodelist.getlistiditem('node' + ff).islive) {
				...
				sonx =
					document.getElementById('reconodediv' + id).offsetLeft - 5
				sony =
					document.getElementById('reconodediv' + id).offsetTop + 15
